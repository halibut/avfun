...
object AudioToImageNNPipelined extends AnimatedCanvasWindow("Sound Test", 200, 200, 20, false, false) { 
  ...
  val networkFile = s"out/${song}-${date}.network.txt"
  val videoFile = s"out/${song}-${date}.mp4"
  ...
  val sizeMult = math.max(1,(sx/100).toInt)
  ...
  .sendTo{case ((dat:Array[Float], frameInfo:FrameInfo)) =>
    val frameDivider = 2
    val length = samplesPerFrame / frameDivider
    
    //Start the image processing pipeline. Each sample for the current frame
    var i = 0
    while(i < length){
      audioSignalPipeline.put(dat, i * frameDivider)
      i+=1
    }
    
    val ginsts = new Array[GInst](length)
...
//    gr.drawString(s"FPS: ${framerate}", 10, 10)
